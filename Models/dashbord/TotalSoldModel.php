...
class TotalSoldModel
{
    ...
    public function getTotalProfitForCurrentMonth()
    {
        try {
            $stmt = $this->query("
                SELECT SUM((COALESCE(products.unit_price, 0) - COALESCE(products.cost_price, 0)) * sale_items.quantity - COALESCE(sale_items.discount, 0)) AS total_profit 
                FROM sale_items 
                LEFT JOIN products ON sale_items.product_id = products.product_id
            ...
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            error_log("Profit Query Result: " . json_encode($result));
    ...
    public function getTotalCostPriceForCurrentMonth()
    {
        try {
            $stmt = $this->query("
                SELECT SUM(COALESCE(products.cost_price, 0) * sale_items.quantity) AS total_cost_price 
                FROM sale_items 
                LEFT JOIN products ON sale_items.product_id = products.product_id
                WHERE MONTH(sale_items.sale_date) = MONTH(CURRENT_DATE())
                AND YEAR(sale_items.sale_date) = YEAR(CURRENT_DATE())
            ...
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            error_log("Cost Price Query Result: " . json_encode($result));
